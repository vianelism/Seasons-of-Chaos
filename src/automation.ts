...
function automaticSlugs(kind: AutomationKind, message: DiscordMessage): string[] {
...
async function announceStamp(env: Env, channelId: string, userId: string, stamp: StampDefinition): Promise<void> {
  await discord(env, `/channels/${channelId}/messages`, { method: "POST", body: JSON.stringify({ content: `**STAMP EARNED 🎉**\n${stamp.emoji} <@${userId}> earned **${stamp.name}**!\n*${stamp.announcement || "The passport office has approved this nonsense."}*` }) });
}

async function announceReward(env: Env, guildId: string, channelId: string, userId: string, reward: Reward): Promise<void> {
  if (reward.roleRewardId) await discord(env, `/guilds/${guildId}/members/${userId}/roles/${reward.roleRewardId}`, { method: "PUT" });
  await discord(env, `/channels/${channelId}/messages`, { method: "POST", body: JSON.stringify({ content: `**REWARD UNLOCKED 🎁**\n${reward.emoji} <@${userId}> unlocked **${reward.name}**!\n*Mom chaos recognized. Completely prestigious.*` }) });
}
...
async function processMessage(env: Env, repository: D1PassportRepository, guildId: string, kind: AutomationKind, announcementChannelId: string, item: DiscordMessage): Promise<number> {
  ...
  if (month === "2026-10" && activity.monthDays >= 3) slugs.push("witch-please");
  if (month === "2026-10" && activity.monthDays >= 5) slugs.push("goblin-mode");
  if (month === "2026-11" && activity.monthDays >= 3) slugs.push("left-no-crumbs");
  if (activity.activeMonths >= 3) slugs.push("i-was-here");
...
export async function runAutomation(env: Env): Promise<void> {
  ...
  for (const channel of channels) {
    if (!channel.last_message_id) {
      const baselineResponse = await discord(env, `/channels/${channel.channel_id}/messages?limit=1`);
    ...
    query.set("after", channel.last_message_id);
    const response = await discord(env, `/channels/${channel.channel_id}/messages?${query}`);
